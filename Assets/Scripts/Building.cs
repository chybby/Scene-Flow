...
/// <summary>
/// Base class for building on the map that holds a Resource inventory and that can be interacted with by Unit.
/// This Base class handles modifying the inventory of resources.
/// </summary>
public abstract class Building : MonoBehaviour, UIMainScene.IUIInfoContent {
    ...
    public int GetItem(string resourceId, int requestAmount) {
        ...
        if (found != -1) {
            ...
            if (m_Inventory[found].Count == 0) {//no more of that resources, so we remove it
